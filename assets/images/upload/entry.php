...
if(EMPTY($_REQUEST["sid"]) || EMPTY($_REQUEST["fname"]) || !preg_match('/^\d{10}$/', $_REQUEST['mno'])){
	echo "<script>alert('Please complete the form.');window.location.href = '".$rd."'; </script>";
}
else
{
	if($_REQUEST["prs"]=="")
	...
	$rs = mysqli_query($con,"SELECT * FROM userloginfor where username='$sid'");
	  ...
	  else
	  {
		  ...
		  $resultlogin = mysqli_query($con,$query);
		  echo "<script>alert('Record Update Success.');window.location.href = '".$rd."'; </script>";
	  }
	}
	else{
		if($auth=="s")
